...
def score_leads(leads: List[Lead]):
    """
    Scores a list of leads and returns the class, probabilities, and top features.
    """
    results = []
    
    # Iterate through each lead in the list
    for lead in leads:
        ...
        predicted_class = clf.predict(lead_df)[0]

        # Compute top 3 contributing features
        # 1. Get preprocessor and classifier from the pipeline
        ...
        pred_class_index = list(classes).index(predicted_class)
